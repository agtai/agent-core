...
| `browser_hover` | 悬停在元素上（tooltip / `:hover` 菜单） |
| `browser_drag` | 从一个元素拖到另一个元素 |
| `browser_drop` | 将本地文件和/或 MIME 数据放到元素上（模拟从页面外拖入；不是元素间拖拽） |
| `browser_press_key` | 按下键盘按键（如 `Enter`、`Escape`） |
| `browser_find` | 在当前无障碍/页面快照中按文本或正则查找；返回匹配的 ref/`target_id`。需先调用 `browser_snapshot` |
...
| `browser_handle_dialog` | 接受或关闭浏览器对话框（alert/confirm/prompt）。阻塞对话框需在触发动作前先调用以武装处理 |
...
模块：`openjiuwen.harness.tools.browser_move.runtime.runtime_tools`
