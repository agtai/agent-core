...
| `metadata.py` | session checkpoint 的 per-team namespace 读写：`read_team_namespace / write_team_namespace / merge_team_namespace / read_team_names_in_session / read_team_db_state / merge_team_db_state / read_pending_resume / merge_pending_resume / clear_pending_resume`。状态结构 `state["teams"][team_name] = {spec, context, model_allocator_state, lifecycle, db_state, pending_resume}`，按 team 分桶；`db_state` 取值为 `pending_create / created / cleaned`；`pending_resume`（`{"query": ...}`）是 leader 的冷恢复续跑标记，`kernel.pause` 写、`kernel.start` 尾部读后即清（见 [[F_61]]）。**dispatch 不读它**——`COLD_RECOVER` 本就是正确的派发结果，续跑发生在 kernel 层 |
