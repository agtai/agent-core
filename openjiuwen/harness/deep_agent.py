    ...
    def _normalize_inputs(self, inputs: Any) -> InvokeInputs:
        ...
        if isinstance(inputs, dict):
            ...
            if run:
                ...
                if context_data:
                    if "source_metadata" in context_data or "_interaction_request_id" in context_data:
                    ...
                    extra.pop("_interaction_request_id", None)
    ...
    async def _send_user(self, request: SendInputRequest) -> Optional[asyncio.Future]:
        ...
        source = RoundWorkItem.user(request_id=request.request_id, inputs=inputs).output_source_metadata(
            ...
            session_id=(self._interaction_session.get_session_id()
                        if self._interaction_session is not None else inputs.get("conversation_id")),
        )
        if source is not None and self._interaction_session is not None:
            self._interaction_session.with_source_metadata(source)
