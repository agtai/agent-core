...
try:
    # Normal package import (openjiuwen.harness.tools.browser_move.runtime.controllers.action)
    from ...shared.env import resolve_upload_root
    from ...shared.parsing import extract_json_object
except ImportError:  # pragma: no cover
    # When imported as top-level `runtime.controllers.action` (runtime/__init__.py
    # appends browser_move/ to sys.path), `...shared` would go beyond the top-level
    # package. Fall back to sibling top-level `shared.*` imports.
    from shared.env import resolve_upload_root
    from shared.parsing import extract_json_object
...
def _build_batch_interact_script(payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "          const contextPages = typeof page.context === 'function' ? page.context().pages() : [page];\n"
        "          const candidates = [page, ...contextPages.filter((candidate) => candidate !== page).reverse()];\n"
        "          for (const candidate of candidates) {\n"
        ...
        "              item.tab_switched = true;\n"
        "            }\n"
        "            return { ok: true, value: currentUrl };\n"
        "          }\n"
        "          return { ok: false, value: String(page.url()) };\n"
...
def register_builtin_actions(controller: ActionController | None = None) -> None:
    ...
    async def browser_batch_interact(**kwargs: Any) -> ActionResult:
        ...
        driver = await _driver_from_controller(ctl)
        if driver is not None:
            from openjiuwen.harness.tools.browser_move.runtime.batch_executor import (
                execute_batch,
            )
