...
"""Browser MCP integration helpers for the browser runtime."""
...
def _runtime_logs_dir() -> Path:
    ...
    if configured:
        ...
        from ...runtime.config import resolve_playwright_mcp_cwd
...
def build_browser_runtime_mcp_config() -> McpServerConfig | None:
    ...
    args = (
        ...
        if args_raw
            ...
            "-m",
            "openjiuwen.harness.tools.browser_move.backends.playwright_mcp.mcp_server",
            "--transport",
            "stdio",
            "--no-banner",
            "--log-level",
