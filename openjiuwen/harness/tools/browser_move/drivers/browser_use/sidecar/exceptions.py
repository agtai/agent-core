...
drivers.errors`` (that would pull the main env's ``openjiuwen`` package, and
...
sharing a class hierarchy. ``drivers/browser_use/sidecar/wire.py`` and
``drivers/errors.py`` are the sources of truth for the code strings; a unit
